...
if (!require(kableExtra)) {
  if (params$`Install needed packages for {thesisdown}`) {
    ...
    stop(
      paste(
        'You need to run install.packages("kableExtra")',
        "first in the Console."
      )
    )
  }
...
This is a `kable` table example below. Any loading and preparing of the table needs to be in separate blocks:
...
knitr::kable(kab, format="latex",
  caption = "An example kable",
  #longtable = TRUE,
  booktabs = TRUE
) %>% kableExtra::kable_styling(latex_options="scale_down") # to rescale the table to fit the width of a page; cannot use with `longtable=TRUE`; see kableExtra::kable_styling for more options
```
...
In the **R** chunk below, we will load in a picture stored as `reed.jpg` in our main directory.  We then give it the caption of "Reed logo", the label of "reedlogo", and specify that this is a figure.  Make note of the different **R** chunk options that are given in the R Markdown file (not shown in the knitted document). Use outwidth to control how wide the figure should be relative the the width of the page.

```{r reedlogo, out.width="80%", fig.cap="Reed logo"}
